...
def mask_estimation(mask, image, score):
    ...
    if mask_percentage < 0.1 and bbox_percentage >= 85:
        print("best quality(should be iris mask)")
        print("image area=" + str(image_area) + "\n" +
              "mask area=" + str(mask_area) + "\n" +
              "mask%= " + str(mask_percentage) + "\n" +
              "mask_bbox%= " + str(mask_bbox_percentage))
    elif mask_percentage < 0.15 and bbox_percentage >= 65:
        print("good quality(iris mask probably)")
        print("image area=" + str(image_area) + "\n" +
              "mask area=" + str(mask_area) + "\n" +
              "mask%= " + str(mask_percentage) + "\n" +
              "mask_bbox%= " + str(mask_bbox_percentage))
    elif mask_percentage < 0.5 and bbox_percentage >= 40:
        print("moderate(eye mask probably)")
        print("image area=" + str(image_area) + "\n" +
              "mask area=" + str(mask_area) + "\n" +
              "mask%= " + str(mask_percentage) + "\n" +
              "mask_bbox%= " + str(mask_bbox_percentage))
